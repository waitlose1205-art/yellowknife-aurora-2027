...
test("server-renders the Yellowknife decision dashboard", async () => {
  ...
  assert.match(html, /<title>黃刀鎮極光旅 2027 決策儀表板<\/title>/i);
  ...
  assert.match(html, /資料匯入檢核/);
  assert.match(html, /跨目的地模板啟用/);
...
test("keeps the finished site free of starter preview wiring", async () => {
  ...
  assert.match(page, /destinationTemplate/);
  ...
  assert.match(css, /\.backlogRank\.first/);
  assert.match(css, /\.enabledGrid/);
  assert.match(css, /\.ruleItem/);
  assert.match(css, /\.templateGrid/);
  ...
  assert.match(layout, /黃刀鎮極光旅 2027 決策儀表板/);
